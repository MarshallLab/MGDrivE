...
The biological parameters were chosen for _Aedes aegypti_ in our original simulations. These parameters represent:  
  ...
  * `betaK`: Daily number of eggs laid per female mosquitoe
...
Our landscape will be a line, with each population only connected to the populations on either side. We set a 5% per day migration rate. There are several examples in the other [vignette](../doc/mgdrive_examples.html) for setting up different landscapes.  
...
The release vector generated is `r length(maleReleasesVector)` elements long, because we have `r releasesParameters$releasesNumber` releases, and each element contains the number and genotype of individuals to release and the time of that release. We now generate the same thing for female releases, and insert the releases into our release list so that releases are only performed in the first patch.  
...
Looking closely, we see the "AAAA" column combined wiht the "AAaa" column in the first output sum to the value in the "AA" column in the second.  
